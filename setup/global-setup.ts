...
const setLocalhostCookie = async (page: Page) => {
  const cookies = await page.context().cookies('https://kaka.dev.nav.no');

  if (!Array.isArray(cookies) || cookies.length === 0) {
    throw new Error('Did not find any cookies for kaka.dev.nav.no');
  }

  if (cookies.length > 1) {
    throw new Error('Found more than one cookie for kaka.dev.nav.no');
  }
